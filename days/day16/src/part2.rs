...
pub fn solve(_input: &str) -> u64 {
    ...
    while let Some(Reverse((cur_cost, state))) = heap.pop() {
        if state.row == end_r && state.col == end_c {
            ...
            let mut states: Vec<State> = Vec::from([state]);
            while !states.is_empty() {
                let current = states.pop().unwrap();
                path.insert((current.row, current.col));
        ...
        if free_cells.contains(&(new_state.row, new_state.col)) {
            ...
            if new_cost < *costs.get(&new_state).unwrap_or(&u64::MAX) {
                ...
                predecessors.insert(new_state, HashSet::from([state]));
            } else if new_cost == *costs.get(&new_state).unwrap_or(&u64::MAX) {
                predecessors.entry(new_state).or_default().insert(state);
            }
        }
