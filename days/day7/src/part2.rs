pub fn solve(_input: &str) -> i64 {
    let ends_with_and_remove = |b: i64, a: i64| -> Option<i64> {
        if a == 0 {
            return None;
        }
        let (mut temp, mut multiplier) = (a, 1);
        while temp > 0 {
            temp /= 10;
            multiplier *= 10;
        }
        if b % multiplier == a {
            return Some(b / multiplier);
        }
        None
    };
    ...
    let possible = |nums: &Vec<i64>, target: i64| -> bool {
        ...
        while !states.is_empty() {
            ...
            if let Some(trimmed_val) = ends_with_and_remove(val, nums[curr_idx]) {
                states.push((trimmed_val, curr_idx - 1));
            }
        }
        return false;
    };
